...
""".lstrip(),
    ...
    resumed = assign_run_config_for_start(config, dry_run=False)
    assert resumed.resuming is True
    assert resumed.config.output_path == first.config.output_path

    complete = run_config(resumed.config, dry_run=True)
    assert complete.executed == 5

    next_run = assign_run_config_for_start(config, dry_run=False)
