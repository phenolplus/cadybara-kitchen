...
@respx.mock
def test_ollama_provider_uses_worker_thread_default(monkeypatch) -> None:
    monkeypatch.setenv("CADYBARA_OLLAMA_NUM_THREAD", "4")
    route = respx.post("http://localhost:11434/api/generate").mock(
        return_value=httpx.Response(
            200,
            json={
    ...
    provider.generate("Prompt", temperature=0.7, max_tokens=128, seed=123)
