...
def render_stl_preview(stl_path: Path, png_path: Path, *, width: int = 640, height: int = 480) -> None:
    import vtk  # noqa: PLC0415 - optional renderer installed with CadQuery/OCP stack.
    ...
    actor.GetProperty().SetColor(0.78, 0.69, 0.55)

    renderer = vtk.vtkRenderer()
    renderer.SetBackground(0.96, 0.93, 0.88)
    ...
    *,
    ...
    if preview_source is not None:
        try:
            ...
            preview_error = f"{exc}\n{traceback.format_exc()}"
            paths["preview_error"].write_text(preview_error, encoding="utf-8")
