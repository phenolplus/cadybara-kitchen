...
function renderStatus(status) {
  ...
  setText(
    ...
    blockers.length
      ? blockers.join(" | ")
      : deps.message || "Prepared voxel dataset and trainer are ready.",
  );
  setText(dom.devicePill, deps.device === "cuda" ? "CUDA" : deps.device === "cpu" ? "CPU voxel" : "missing deps");

  setText(dom.examples, formatNumber(dataset.train_count));
  setText(dom.test, `${formatNumber(dataset.test_count)} test | ${formatNumber(dataset.val_count)} val`);
  ...
  setText(dom.modelDir, shortPath(status.latest_sample_stl || status.model_dir || defaults.model_dir));
  ...
  setText(
    ...
    dataset.manifest
      ? `${formatNumber(dataset.manifest.written)} voxelized from ${formatNumber(dataset.manifest.scanned_obj)} OBJ`
      : dataset.data_dir || "missing",
  );
  setText(dom.readinessGrammar, `${formatNumber(defaults.resolution || dataset.resolution)}³ target`);
  setText(dom.readinessTorch, `${deps.versions?.torch || "missing torch"} | ${deps.message || ""}`.trim());
  setText(
    dom.readinessDefaults,
    `${formatNumber(defaults.batch_size)} batch | ${formatNumber(defaults.base_channels)} channels | seed ${defaults.seed}`,
  );
...
async function refreshStatus() {
  const status = await getJson("/api/voxel-diffusion/status");
  renderStatus(status);
}
...
async function startTraining() {
  ...
  try {
    await postJson("/api/voxel-diffusion/train/start", {});
    await refreshStatus();
  } catch (error) {
...
async function stopTraining() {
  ...
  try {
    await postJson("/api/voxel-diffusion/train/stop", {});
  } finally {
    await refreshStatus().catch(() => {});
  }
}
